    ...
    if([userName isEqualToString:@""]||userName==nil)
    {
        ...
        if([password isEqualToString:@""]||password==nil)
        {
            ...
            else
            {
                ...
                [user setValue:[NSNumber numberWithInt:180] forKey:@"height"];
                [user setValue:[NSNumber numberWithFloat:70] forKey:@"weight"];
                [user setValue:[dateFormatter dateFromString:@"1988-2-10"] forKey:@"birthDate"];
                @try {
                    ...
                    //result=YES;
                    //[[EzonAppDelegate getLoginViewController].navigationController pushViewController:[EzonAppDelegate getLoginViewController] animated:YES];
                    //[self showMainView];
                    //注册成功：保存全局用户信息
